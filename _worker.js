...
const manualPipe = async (readable, writable, close) => {
    const safeBufferSize = bufferSize - maxChunkLen, halfChunkLen = maxChunkLen >> 1, fastFlushOffset = bufferSize >> 1;
    ...
    let directChunks = [], directBytes = 0, directTimer = null, isClose = false, fastFlush = true;
    const flushDirect = () => {
        ...
        if (directBytes > 0 && !isClose) {
            if (directChunks.length === 1) {
                ...
                let directOff = 0;
                for (const chunk of directChunks) merged.set(chunk, directOff), directOff += chunk.byteLength;
                writable.send(merged);
            }
        }
        directChunks = [], directBytes = 0;
    };
    const flushBuffer = () => {
        if (isReading) return needsFlush = true;
        fastFlush = offset < fastFlushOffset;
        offset > 0 && !isClose && (offset > safeBufferSize ? (writable.send(bufferView.subarray(0, offset)), fastFlush || (buffer = new ArrayBuffer(bufferSize), bufferView = new Uint8Array(buffer))) : writable.send(bufferView.slice(0, offset)), offset = 0);
        ...
        while (true) {
            if (fastFlush) {
                ...
                if (chunkLen >= halfChunkLen) {
                    ...
                    directChunks.push(value.slice()), directBytes += chunkLen, spareBuffer = value.buffer, flushDirect();
                } else {
                    directChunks.push(value.slice()), directBytes += chunkLen, directTimer ||= setTimeout(flushDirect, 1), spareBuffer = value.buffer;
                }
                if (chunkLen < 28672) {
                    totalBytes = 0;
                } else if ((totalBytes += chunkLen) > startThreshold) {
                    flushDirect(), fastFlush = false, buffer = new ArrayBuffer(bufferSize), bufferView = new Uint8Array(buffer);
                }
            } else {
